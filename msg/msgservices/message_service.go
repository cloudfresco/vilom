...
func (t *MessageService) Create(ctx context.Context, form *Message, UserID string, rplymsg bool, userEmail string, requestID string) (*Message, error) {
	...
	default:
		if form.Mtext != "" {
			...
			_, err = t.CreateMessageText(ctx, tx, form, Message.ID, user.ID, userEmail, requestID)
			...
			_, err = t.CreateMessageAttachment(ctx, tx, form, Message.ID, user.ID, userEmail, requestID)
			...
			if rplymsg {
				err = t.CreateUserReply(ctx, tx, form.TopicID, Message.ID, user.ID, form.UgroupID, userEmail, requestID)
				if err != nil {
...
func (t *MessageService) CreateMessageAttachment(ctx context.Context, tx *sql.Tx, form *Message, messageID uint, userID uint, userEmail string, requestID string) (*MessageAttachment, error) {
	...
	default:
		...
		msgathmnt, err := t.InsertMessageAttachment(ctx, tx, msgath, userEmail, requestID)
